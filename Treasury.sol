...
contract GoblinTreasury is Ownable {
    ...
    address public sGoblin;
    uint public sGOBQueue; // Delays change to sGOB address
    ...
    //    isLiquidityToken[ _GOBDAI ] = true;
    //    liquidityTokens.push( _GOBDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for GOB
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint GOB needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Goblin ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn GOB for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IGOBERC20( Goblin ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sGoblin ).balanceOf( msg.sender ); // Can only borrow against sGOB held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with GOB
        @param _amount uint
     */
    function repayDebtWithGOB( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IGOBERC20( Goblin ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Goblin, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Goblin ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Goblin ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns GOB valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match GOB decimals
            value_ = _amount.mul( 10 ** IERC20( Goblin ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SGOB ) { // 9
            sGOBQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
